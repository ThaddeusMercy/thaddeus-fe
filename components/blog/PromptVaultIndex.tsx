...
function collectionHref(linkBase: PromptVaultIndexProps["linkBase"]) {
  if (linkBase === "blog_route") {
    return `/blog/prompt-vault/${PROMPT_VAULT_COLLECTION.slug}`;
  }
  return guideHubHref("prompts", { post: PROMPT_VAULT_COLLECTION.slug });
}
...
export default function PromptVaultIndex({
  ...
  const href = collectionHref(linkBase);
    ...
    >
      <header className="space-y-4 border-b border-border pb-8">
        ...
        <p className="leading-relaxed">
          One post, ten prompts — open it, scroll, copy what you need.
        </p>
      </header>
      ...
      <ul className="divide-y divide-border rounded-2xl border border-border bg-white">
        <li>
          <Link
            ...
            onClick={() =>
              trackEvent("resources_prompt_open", {
                ...
                prompt_slug: PROMPT_VAULT_COLLECTION.slug,
          ...
          >
            ...
            >
              <p className="font-mono text-xs text-[#D8D8D8]">01</p>
              ...
              <p className="text-sm leading-relaxed text-[#676767]">
                {PROMPT_VAULT_COLLECTION.excerpt}
              </p>
            </motion.div>
